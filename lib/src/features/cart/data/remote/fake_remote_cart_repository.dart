...
class FakeRemoteCartRepository implements RemoteCartRepository {
  ...
  final _carts = InMemoryStore<Map<String, Cart>>({});
   final _shoppingCarts = InMemoryStore<Map<String, ShoppingCart>>({});
  ...
  @override
  Future<void> setShoppingCart(String uid, ShoppingCart cart) async{
    // TODO: implement setShoppingCart
     await delay(addDelay);
    // First, get the current carts data for all users
    final carts = _shoppingCarts.value;
    // Then, set the cart for the given uid
    carts[uid] = cart;
    // Finally, update the carts data (will emit a new value)
  ...
  Future<ShoppingCart> fetchShoppingCart(String uid) {
    // TODO: implement fetchShoppingCart
    return Future.value(_shoppingCarts.value[uid] ?? const ShoppingCart());
  }
}
